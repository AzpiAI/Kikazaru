...
async function loadLanguage(select) {
	...
	if (voice.modelStatus !== "unloaded") voice.unloadModel();

	//load the selected model
	var lang = select[select.selectedIndex].value;
	loadingLanguage(true);
	await voice.loadModel(lang, window.models[lang].url);
...
function changeStatus(status) {
	...
	document.getElementById(
		"statusPanel"
	).innerHTML += `<span id="currentStatus" data-i18n="status.${status}" class="status-${status}"></span>`;
